...
def generate_and_save_data():
    ...
    x.astype(np.float32).tofile('S01.bin')   #写入
    print("Save as S01.bin ,Size:", len(x) * 4, "Byte")
    return k, x


def load_and_plot():
    #读取与载入范围
    recovered_x = np.fromfile('S01.bin', dtype=np.float32)   #读取
